...
class authenticate_user():
    ...
    def check_if_user_has_token(self,user_token):
        data = self.read_jwts_file()
        ...
        for item in data:
          if payload.get("jti") == item.get("jti") and payload.get("sub") == item.get("username") and payload.get("exp") == item.get("exp") and  datetime.now(timezone.utc).timestamp() < item.get("exp"):
